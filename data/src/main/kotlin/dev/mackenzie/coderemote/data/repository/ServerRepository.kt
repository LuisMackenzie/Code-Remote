 ...
 *
 * Uses [KeyValueStorage] to persist server configurations.
 */
@Singleton
class ServerRepository @Inject constructor(
     ...
     */
    val servers: Flow<List<ServerConfig>> = storage.observeString(SERVERS_KEY, "[]").map { serversJson ->
        try {
            json.decodeFromString<List<ServerConfig>>(serversJson)
        } catch (e: Exception) {
            logger.e(TAG, "Failed to decode servers", e)
            emptyList()
        }
    }
    ...
    suspend fun checkHealth(server: ServerConfig): Result<ServerHealth> {
        return try {
            ...
            logger.e(TAG, "Health check failed for ${server.url}", e)
    ...
    private suspend fun saveServers(servers: List<ServerConfig>) {
        storage.putString(SERVERS_KEY, json.encodeToString(servers))
    }
}
